...
class TrainerCommand(Command):
    ...
    def __init__(self, top_parser, subparser):
        ...
        self.flags = [
            {
                'command': '--train-file',
                'help': 'JSON or CSV solve file, with all the parameters to start the training.',
                'type': str, 'metavar': 'TRAIN_FILE_PATH', 'action': 'store',
            },
            # TODO
            #  {
            #  'command': '--build-training-file',
            #  'help': 'Helper to build a solve json-file.',
            #  'action' : 'store_true'
            #  },
            {
                'command': '--convert',
                'help': 'Training file to convert. Must be used with --output-format option.',
                'action': 'store',
            },
            {
                'command': '--output-format',
                'help': 'Converted file format . Must be used with --convert option.'
                        + " Accepted values are 'CSV', 'YAML' and 'JSON'.",
                'action': 'store',
            },
            {
                'command': '--play',
                'help': 'Test agent, without training it, it will ignore train entry on json file.',
                'action': 'store_true',
            },
            {
                'command': '--threaded',
                'help': 'If there is more than one training in the file, it will be executed in a'
                        + ' separated thread.',
                'action': 'store_true',
            },
        ]
    ...
    def run(self, args):
        ...
        # elif args.build_training_file:
            ...
            if args.output_format is not None:
                ...
                output_path = (os.path.abspath(os.path.dirname(args.convert))
                               + os.path.sep
                               + os.path.splitext(os.path.basename(args.convert))[0]
                               + '.file_format')
            ...
            raise Exception(
                'You must specify --train-file or --convert (with --output-format set).')
