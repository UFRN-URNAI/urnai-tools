...
class SC2Command(Command):
    ...
    def __init__(self, top_parser, subparser):
        ...
        self.flags = [
            {
                'command': '--sc2-map', 'help': 'Map to use on Starcraft II.',
                'type': str,
                'metavar': 'MAP_FILE_PATH',
                'action': 'store',
            },
            {
                'command': '--extract-specs',
                'help': 'This will export every map layer to a csv file. Other userful'
                        + ' information will be on a JSON file. This switch works with'
                        + ' both sc2 and drts commands.',
                'action': 'store_true',
            },
        ]
    ...
    #         np.savetxt('feature_minimap_layer_{}.csv'.format(cont), map_csv, fmt='%i',
    #                    delimiter=',')
    #         cont += 1
