...
class NeuralNetworkFactory:
    ...
    @classmethod
    def get_nn_model(self, action_size, state_size, build_model, lib=constants.Libraries.KERAS,
                     gamma=0.99, learning_rate=0.001, seed_value=None, batch_size=32, lamb=0.9):
        if lib in constants.listoflibs:
            nn = None

            if lib == constants.Libraries.KERAS:
                # importing this way is safer because it uses importlib
                cls = get_cls('urnai.models.memory_representations.neural_network',
                              'KerasDeepNeuralNetwork')
            ...
            # elif shortens code execution
                ...
                cls = get_cls('urnai.models.memory_representations.neural_network',
                              'KerasDNNEligibilityTraces')
                nn = cls(action_size, state_size, build_model, gamma, learning_rate, seed_value,
                         batch_size, lamb, True)
            return nn
        else:
