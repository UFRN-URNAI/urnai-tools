class ModelBuilder:
    ...
    def add_input_layer(self, nodes=50):
        ...
        # else:
        #     raise TypeError(
        #            'Input layer shape should be a list with its dimensions in it.')

    def add_convolutional_layer(self, filters=2, kernel_size=(3, 3), strides=(1, 1),
                                padding='valid',
        ...
        if name == 'default':
            ...
            for layer in self.layers:
                if 'name' in layer and 'default' in layer['name']:
                        cont += 1
    ...
    def add_maxpooling_layer(self, pool_size=(2, 2), strides=None, padding='valid',
                             data_format=None, name='default'):
        if name == 'default':
            cont = 0
            for layer in self.layers:
                if 'name' in layer and 'default' in layer['name']:
    ...
    def add_fullyconn_layer(self, nodes, name='default'):
        ...
        if isinstance(nodes, int):
            ...
            raise TypeError(
                "Fully connected layer's number of nodes should be an integer.")
    ...
    def is_last_conv_layer(layer, layers):
        index = ModelBuilder.get_last_convolutional_layer_index(layers)
        return index == layers.index(layer)
