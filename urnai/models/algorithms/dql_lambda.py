...
class DeepQLearningLambda(LearningModel):
    """
    ...
    def __init__(self, action_wrapper: ActionWrapper, state_builder: StateBuilder,
                 learning_rate=0.001, learning_rate_min=0.0001, learning_rate_decay=1,
                 learning_rate_decay_ep_cutoff=0, gamma=0.99, name='DeepQLearning',
                 build_model=None, epsilon_start=1.0, epsilon_min=0.005, epsilon_decay=0.99995,
                 per_episode_epsilon_decay=False, memory_maxlen=50000, batch_size=32,
                 min_memory_size=2000, seed_value=None, cpu_only=False, lib='keras_e_traces',
                 neural_net_class=None, epsilon_linear_decay=False, lr_linear_decay=False,
                 lamb=0.9):
    ...
    def learn(self, s, a, r, s_, done):
        ...
        batch = [0] * self.batch_size  # list of size batch_size full of zeros
        batch_idx = random.randint(self.batch_size,
                                   len(self.memory) - 1)  # choosing random point in self.memory
        for i in range(0, self.batch_size):
            batch[i] = self.memory[
                batch_idx - i]  # building a batch of sequential tuples in memory in reverse order

        # batch = random.sample(self.memory, self.batch_size)
        states = numpy.array([val[0] for val in batch])
        states = numpy.squeeze(states)
        next_states = numpy.array(
            [(numpy.zeros(self.state_size) if val[3] is None else val[3]) for val in batch])
        ...
        # in contrast to batch training, we have to do
        # this individually so that the eligibility traces
        # algorithm can diminish the trace between each step
        for i in range(0, self.batch_size):
            state = numpy.array([states[i]])
            target = numpy.array([target_q_values[i]])
            self.dnn.update(state, target)

        # self.dnn.update(states, target_q_values)

        self.dnn.reset_e_trace()
