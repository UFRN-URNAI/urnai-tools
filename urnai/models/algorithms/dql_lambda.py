...
class DeepQLearningLambda(LearningModel):
    ...
    def __init__(self, action_wrapper: ActionWrapper, state_builder: StateBuilder, learning_rate=0.001, learning_rate_min=0.0001, learning_rate_decay=1, 
                ...
                per_episode_epsilon_decay=False, memory_maxlen=50000, batch_size=32, min_memory_size=2000, seed_value=None, cpu_only=False, lib='keras_e_traces', neural_net_class=None, epsilon_linear_decay=False, lr_linear_decay=False, lamb=0.9):
        ...
        if neural_net_class != None:
            self.dnn = neural_net_class(self.action_size, self.state_size, self.build_model, self.gamma, self.learning_rate, self.seed_value, 1, self.lamb)
        else:
            self.dnn = NeuralNetworkFactory.get_nn_model(self.action_size, self.state_size, self.build_model, self.lib, self.gamma, self.learning_rate, self.seed_value, 1, self.lamb)
    ...
    def learn(self, s, a, r, s_, done):
        ...
        batch_idx = random.randint(self.batch_size, len(self.memory)-1) # choosing random point in self.memory
        for i in range(0, self.batch_size):
            batch[i] = self.memory[batch_idx - i] # building a batch of sequential tuples in memory in reverse order

        # batch = random.sample(self.memory, self.batch_size)
        states = numpy.array([val[0] for val in batch])
        states = numpy.squeeze(states)
        next_states = numpy.array([(numpy.zeros(self.state_size) if val[3] is None else val[3]) for val in batch])
