...
class PGKeras(LearningModel):
    ...
    def __init__(self, action_wrapper: ActionWrapper, state_builder: StateBuilder, gamma=0.95,
                 learning_rate=0.002, learning_rate_min=0.0002, learning_rate_decay=0.99995,
        ...
        super(PGKeras, self).__init__(action_wrapper, state_builder, gamma, learning_rate,
                                      learning_rate_min, learning_rate_decay, name, epsilon_min=0.1,
                                      epsilon_decay_rate=0.995, learning_rate_decay_ep_cutoff=0,
    ...
    def make_model(self):
        ...
        for layer_model in self.build_model:
            model_layers_len = len(model_layers)
            if model_layers_len >= 0:
                prev_idx = model_layers_len - 1
            else:
                prev_idx = 0
            ...
            if layer_model['type'] == ModelBuilder.LAYER_INPUT:
                ...
                model_layers.append(
                    Dense(layer_model['nodes'], activation='relu')(model_layers[prev_idx]))

            elif layer_model['type'] == ModelBuilder.LAYER_OUTPUT:
                model_layers.append(
                    Dense(layer_model['length'], activation='softmax')(model_layers[prev_idx]))
                ...
                if self.build_model.index(layer_model) == 0:
                    ...
                    model_layers.append(Conv2D(layer_model['filters'], layer_model['filter_shape'],
                        ...
                        model_layers[0]))
                    model_layers.append(
                        MaxPooling2D(pool_size=layer_model['max_pooling_pool_size_shape'])(
                            model_layers[1]))
                else:
                    model_layers.append(Conv2D(layer_model['filters'], layer_model['filter_shape'],
                                               padding=layer_model['padding'], activation='relu')(
                        model_layers[prev_idx]))
                    prev_idx += 1
                    model_layers.append(
                        MaxPooling2D(pool_size=layer_model['max_pooling_pool_size_shape'])(
                            model_layers[prev_idx]))
