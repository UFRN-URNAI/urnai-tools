...
from tensorflow.compat.v1 import ConfigProto, global_variables_initializer, layers, placeholder, \
    Session, train
...
class DqlTensorFlow(LearningModel):
    ...
    # of Deep-Q Learning
    def __init__(self, action_wrapper: ActionWrapper, state_builder: StateBuilder,
                 learning_rate=0.0002, learning_rate_min=0.00002, learning_rate_decay=1,
                 learning_rate_decay_ep_cutoff=0, gamma=0.95, name='DQN',
                 build_model=ModelBuilder.DEFAULT_BUILD_MODEL, epsilon_start=1.0, epsilon_min=0.5,
                 epsilon_decay=0.995, per_episode_epsilon_decay=False, use_memory=False,
                 memory_maxlen=10000, batch_training=False, batch_size=32, min_memory_size=5000,
                 seed_value=None, cpu_only=False, epsilon_linear_decay=False,
                 lr_linear_decay=False):
        super().__init__(action_wrapper, state_builder, gamma, learning_rate, learning_rate_min,
                         learning_rate_decay, epsilon_start, epsilon_min, epsilon_decay,
                         per_episode_epsilon_decay, learning_rate_decay_ep_cutoff, name, seed_value,
    ...
    def memory_learn(self, s, a, r, s_, done):
        ...
        for i, (state, action, reward, next_state, done) in enumerate(batch):
            ...
            current_q = q_s_a[i]
            if done:
                # if this is the last step, there is no future max q value, so we
                # the new_q is just the reward
                current_q[action] = reward
            else:
                # new Q-value is equal to the reward at that step + discount factor *
                # the max q-value for the next_state
