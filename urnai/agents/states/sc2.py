...
from urnai.agents.actions.sc2 import get_my_units_amount, get_unit_amount, get_units_by_type
from urnai.utils.image import lower_featuremap_resolution
from urnai.utils.sc2_utils import get_fog_of_war_percentage
...
class Simple64State(StateBuilder):
    ...
    def build_state(self, obs):
        ...
        # Feature layer of creep in the minimap (generally will be quite empty, especially on
        # games without zergs hehe)
        m1 = obs.feature_minimap[2]
        m1[m1 == 1] = 16  # Transforming creep info from 1 to 16 in the map (makes it more visible)

        m2 = obs.feature_minimap[
            4]  # Feature layer of all visible units (neutral, friendly and enemy) on the minimap
        ...
        # Overlaying the m2(units) map over the m1(creep) map
        combined_minimap = np.where(m2 != 0, m2, m1)
        # Overlaying the combined m1 and m2 map over the m0(terrain) map
        combined_minimap = np.where(combined_minimap != 0, combined_minimap, m0)
        # combined_minimap = m0+m1+m2
        # Tranforming combined_minimap into a np array so tensor flow can interpret it
        ...
        # featuremap and reduction factor, if rf = 4 a 64x64 map will be transformed into a
        # 16x16 map
        lowered_minimap = lower_featuremap_resolution(combined_minimap, self.reduction_factor)

        # Rotating observation depending on Agent's location on the map so that we get a
        # consistent, generalized, observation
        ...
        # Displaying the agent's vision in a heatmap using matplotlib every 200 steps (just for
        # debug purpuses, probably will be removed later)
...
class Simple64StateFullRes(StateBuilder):
    ...
    def build_state(self, obs):
        ...
        m1 = obs.feature_minimap[2]
        m1[m1 == 1] = 16  # Transforming creep info from 1 to 16 in the map (makes it more visible)
        # Feature layer of all visible units (neutral, friendly and enemy) on the minimap
        m2 = obs.feature_minimap[4]
        m2[m2 == 1] = 128  # Transforming own units from 1 to 128 for visibility
        m2[m2 == 3] = 64  # Transforming enemy units from 3 to 64 for visibility
        m2[m2 == 2] = 64
        m2[m2 == 16] = 32  # Transforming mineral fields and geysers from 16 to 32 for visibility
        ...
        # Displaying the agent's vision in a heatmap using matplotlib every 200 steps
        # (just for debug purpuses, probably will be removed later)
...
class Simple64GridState(StateBuilder):
    ...
    def build_state(self, obs):
        ...
        if self.player_race == sc2_env.Race.terran:
            ...
            # Insteading of making a vector for all coordnates on the map, we'll
...
class Simple64GridState_SimpleTerran(StateBuilder):
    ...
    def build_state(self, obs):
        ...
        # Normalizing the values to always be between 0 and 1 (since the max
        # amount of units in SC2 is 200)
        enemy_grid = enemy_grid / 200
        player_grid = player_grid / 200
...
class SimpleCroppedGridState(StateBuilder):
    def __init__(self, x1=12, y1=12, x2=50, y2=50, grid_size=4, r_enemy=True, r_player=True,
                 r_neutral=False):
        self.grid_size = grid_size
        self.x1 = x1
        self.y1 = y1
    ...
    def build_state(self, obs):
        state = build_cropped_gridstate(obs, self.grid_size, self.x1, self.y1, self.x2, self.y2,
                                        self.r_enemy, self.r_player, self.r_neutral)
        return state
...
class UnitStackingState(StateBuilder):
    """
    This state builder creates an input matrix with dimensions (?,4), where the first few lines
    of the input matrix are
    non-spatial features (minerals, vespene, idle worker count, game alerts, max supply etc) and
    the remaining lines are
    divided into two groups of unit features: one group for the player, and another for the enemy.

    The two groups of unit features are further divided into two groups: the group of units we
    only want to know the amount of,
    and the group of units we want to know the amount, avg health ratio and avg position. The
    first group is called 'amount_unit_types'
    while the second is called 'spatial_unit_types'. 'amount_unit_types' is defined as a list of
    PySC2 unit types, while
    'spatial_unit_types' is defined as a list of lists of PySC2 unit types, both defined in the
    __init__() method.

    'spatial_unit_types'is defined as a list of lists so that we can group certain types of units
    together and treat them like one type.
    For example, we could group units based on their traversal medium (separate ground from air
    units), or we could group them based on
    their attacking capabilities (all air-attacking units together, etc), or we could group them
    using both criteria (all ground units
    with ground and air attacking capabilities together, etc). There is really no end to
    combinations here, and each developer can
    choose how to implement their version of this class to achieve their desired observation
    grouping.

    After both the player and enemy units are grouped, and their propeties calculated, we flatten
    out the output, so that a
    ...
    def build_state(self, obs):
        """
        State shape is defined as follows (3,4) for non-spatial features, like minerals, gas,
        supply, etc,
        then (a,4) for amount_unit_types, where 'a' is the size of amount_unit_types divided by 4.
        If amount_unit_types has 12 unit types, we need 3 matrix lines to store them, so 'a'
        will be 3.
        ...
        # find a way of getting max_steps_training here?
        state[2][2] = (obs.game_loop[-1] / obs.step_mul) / 2000
        state[2][3] = get_fog_of_war_percentage(obs)

        # addind amount of each unit in amount_unit_types for the player to state
        state = build_unit_amount_matrix(obs, features.PlayerRelative.SELF, state,
                                         self.amount_unit_types, 4, 20)

        # creating matrix of spatial features for the player's unit groups
        state = build_unit_feature_matrix(obs, features.PlayerRelative.SELF, state,
                                          self.spatial_unit_types, 200)

        # addind amount of each unit in amount_unit_types for the enemy to state
        state = build_unit_amount_matrix(obs, features.PlayerRelative.ENEMY, state,
                                         self.amount_unit_types, 4, 20)

        # creating matrix of spatial features for the enemy's unit groups
        state = build_unit_feature_matrix(obs, features.PlayerRelative.ENEMY, state,
                                          self.spatial_unit_types, 200)
...
class TVTUnitStackingState(UnitStackingState):
    ...
    def __init__(self):
        ...
        self.spatial_unit_types = [
            ...
            # Ground -> Ground
            [units.Terran.Reaper, units.Terran.Marauder, units.Terran.SiegeTank,
        ...
        self._state_size = 4 * (3 + 2 * (
                math.ceil(len(self.amount_unit_types) / 4) + len(self.spatial_unit_types)))
...
class TVTUnitStackingEnemyGridState(UnitStackingState):
    ...
    In this class we also use a grid for enemy units (to help the agent locate enemy
    buildings on map)
    """
    ...
    def __init__(self, grid_size=4):
        ...
        self._state_size = 4 * (3 + 2 * (math.ceil(len(self.amount_unit_types) / 4) + len(
            self.spatial_unit_types))) + self.grid_size * self.grid_size
    ...
    def build_state(self, obs):
        ...
        # then we add all of them to the proper square in the grid (note that this adds
        # units that have already been added by the first
...
class MultipleUnitGridState(StateBuilder):
    def __init__(self, grid_size=4):
        ...
        self.list_unit_types = [
            ...
            [units.Terran.Marine, units.Terran.Cyclone, units.Terran.WidowMine, units.Terran.Reaper,
             units.Terran.Marauder, units.Terran.SiegeTank, units.Terran.Hellion,
            ...
            # Any unit or construction that is not contemplated by the lists above
            # will be grouped together in a final list
            # automatically. So, supply depots, refineries, engineering bays etc
            # will all be grouped together.
        ]
        self._state_size = 20 + (self.grid_size * self.grid_size * 2) * (
                len(self.list_unit_types) + 1)
    ...
    def build_state(self, obs):
        ...
        # find a way of getting max_steps_training here?
        state = np.append(state, (obs.game_loop[-1] / obs.step_mul) / 2000)
        ...
        build_multiple_unit_grid(obs, features.PlayerRelative.SELF, player_grid, self.grid_size,
                                 self.list_unit_types)

        enemy_grid = np.zeros((self.grid_size, self.grid_size, len(self.list_unit_types) + 1))
        build_multiple_unit_grid(obs, features.PlayerRelative.ENEMY, enemy_grid, self.grid_size,
                                 self.list_unit_types)
...
def build_multiple_unit_grid(obs, player, grid, grid_size, unit_groups):
    for group_i, unit_group in enumerate(unit_groups):
        ...
        group_units = [unit for unit in obs.raw_units if
                       unit.alliance == player and unit.unit_type in unit_group]
    ...
    # after adding all units from the types in unit_groups we need to add the remaining units
    # whose types are not in unit_groups
    ...
    # then we add all of them to the proper square in the grid (note that this adds units that
    # have already been added by the first
    # for, since we don't filter out overall_units by unit type)
    ...
    for unit_group in spatial_unit_types:
        ...
        for unit_type in unit_group:
            units = [unit for unit in obs.raw_units if
                     unit.alliance == player and unit.unit_type == unit_type]
...
def build_cropped_gridstate(obs, grid_size, x1, y1, x2, y2, r_enemy: bool, r_player: bool,
                            r_neutral: bool):
    """
    This function generates a series of grids based on a cropped raw SC2 representation.
    You can have a return vector with only the enemy, player or neutral units, or any
    mix of these three.
    The cropping logic is the following: you could have a 64x64 SC2 map but the
    playable part is restricted
    to a rectangle defined by the top-left point with x1=20 and y1=25 and the bottom
    right point with x2=40, y2=45.
    The points (x1,y1) and (x2,y2) will be used to calculate the position of units
    relative to the playable area.
    This would, for example, mean that a unit that originally was on the (24,27) point
    will now be on the (4,2) on our cropped representation.

    Args:
        obs: raw SC2 observation
        grid_size (int):
        x1 (int): is the x position of the top-left point of the rectangle that you want to
          crop the map
        y1 (int): is the y positiono f the top-left point of the rectangle that you want to
          crop the map
    ...
    Returns:
        A list with the flattened grids from the enemy, player and neutral units depending on the
        input arguments.
    """
    new_state = []
    ...
    if r_enemy:
        ...
        # Normalizing the values to always be between 0 and 1 (since the max amount of
        # units in SC2 is 200)
    ...
    if r_neutral:
        ...
        neutral_units = [unit for unit in obs.raw_units if
                         unit.alliance == features.PlayerRelative.NEUTRAL]
