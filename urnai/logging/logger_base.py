...
class LoggerBase(ABC):
    ...
    This should be overridden by subclasses to specify their own LoggingModeBase
    enum class. Defaults to base LoggingModeBase."""
    ...
    def set_mode(self, mode: str | LoggingModeBase) -> None:
        ...
        :param mode: A LoggingModeBase instance representing either
                     TRAINING or EVALUATION mode.
        """
        self._mode_cls.check(mode)
        self.mode = mode
        if isinstance(mode, str):
            self.mode = self._mode_cls[mode.upper()]
