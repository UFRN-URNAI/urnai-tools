...
def get_urnai_latest_commit_hash():
    ...
    if git_path is not None:
        print(git_path)
        latest_commit_hash = f'{git_path}{sep}refs{sep}heads{sep}master'
        ...
        raise Errors.UrnaiRepoNotFoundError('Urnai repo: file not found!')
...
def find_urnai_git_repo_dir():
    ...
    if os.exists(git_path):
        ...
        answer = get_string_input(
            'I could not find urnai git repo, do you want me to'
            + ' search recursively on your home folder? [y/n]',
            'n').lower()
        if answer == 'y':
            ...
            if len(urnai_repos) > 0:
                if len(urnai_repos) == 1:
                    ...
                    for i in range(len(urnai_repos)):
                        print(f'{i + 1} - {urnai_repos[i]}')
...
def generate_log(user, pip_freeze, output, err):
    with open('geral_test_info.log', 'w') as file_:
        text = """User: {usr}
        Packages installed (pip freeze): {pip_fr}""".format(usr=user,
                                                            pip_fr=str(pip_freeze, 'UTF-8'))
        file_.write(text)
...
def main():
    ...
    output, err = '', ''
    p = subprocess.Popen(['python', '-m', 'pytest', 'test_cartpole.py'], stdout=subprocess.PIPE,
