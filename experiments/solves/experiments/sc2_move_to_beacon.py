...
from urnai.agents.rewards.sc2 import MoveToBeaconProximity
from urnai.agents.states.sc2 import MoveToBeaconGridState
...
def declare_trainer(): 
    env = SC2Env(map_name="MoveToBeacon", render=False, step_mul=8, player_race="terran", enemy_race="terran", difficulty="very_easy")

    map_size = env.env_instance._interface_formats[0]._raw_resolution
    action_wrapper = MoveToBeaconActionWrapper(10, 10, map_size.x, map_size.y)
    state_builder = MoveToBeaconGridState(grid_size=8)

    helper = ModelBuilder()
    helper.add_input_layer(nodes=40)
    ...
    trainer = Trainer(env, agent, save_path='../agentes-treinados-tcc', file_name="movetobeacon_8x8obs_10x10act",
                    save_every=100, enable_save=True, relative_path=True, reset_epsilon=False,
                    max_training_episodes=700, max_steps_training=600,
