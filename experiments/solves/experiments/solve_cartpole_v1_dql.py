...
def declare_trainer():
    ...
    dq_network = DeepQLearning(action_wrapper=action_wrapper, state_builder=state_builder, build_model=helper.get_model_layout(), gamma=0.99, use_memory=True, 
                learning_rate=0.001, epsilon_decay=0.9997, epsilon_min=0.01, memory_maxlen=50000, min_memory_size=100, batch_size=32, lib="pytorch")
    ...
    dq_network = DeepQLearningLambda(action_wrapper=action_wrapper, state_builder=state_builder, build_model=helper.get_model_layout(),
                    lamb=0.9, gamma=0.99, learning_rate=0.001, memory_maxlen=128, min_memory_size=64, lib="keras_e_traces",
    ...
    trainer = Trainer(env, agent, save_path='urnai/models/saved', file_name="cartpole_v1_dql_pytorch1", 
