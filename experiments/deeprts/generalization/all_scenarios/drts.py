...
import numpy as np
from urnai.scenarios.generalization.rts.defeatenemies import \
    GeneralizedDefeatEnemiesScenario as Scenario
...
Lumber: {l}
Food: {f}
Number of archers: {na}""".format(
                player=idx + 1,
                o=player.oil,
                g=player.gold,
                l=player.lumber,
                f=player.food,
                na=player.num_archer,
            )
            print(player_stats)
        ...
        if print_collectables_map:
            if 'collectables_map' in env.__dict__.keys():
                ...
                np.savetxt(
                    os.path.expanduser('~') + os.path.sep + 'curr_coll_map.csv',
                    a,
                    fmt='%i',
                    delimiter=',',
                )
