...
SCENARIO_LIST = [GeneralizedCollectablesScenario, GeneralizedFindAndDefeatScenario,
                 GeneralizedDefeatEnemiesScenario, GeneralizedBuildUnitsScenario]
GAME_LIST = [GeneralizedCollectablesScenario.GAME_DEEP_RTS,
    ...
    for Scenario in SCENARIO_LIST:
        ...
        for ep in range(episodes):
            ...
            for step in range(steps):
                ...
                if not env_reward:
                    reward = reward_builder.get_reward(state)
                if not env_state:
                    state = state_builder.build_state(state)
            ...
            Lumber: {l}
            Food: {f}
            Number of archers: {na}""".format(player=idx + 1, o=player.oil, g=player.gold,
                                              l=player.lumber, f=player.food, na=player.num_archer)
                        print(player_stats)

                    if print_collectables_map:
                        if 'collectables_map' in env.__dict__.keys():
                            a = env.collectables_map.astype(int)
                            np.savetxt(os.path.expanduser('~') + os.path.sep + 'curr_coll_map.csv',
                                       a, fmt='%i', delimiter=',')
                else:
                    if PRINT_MAP:
                ...
                if SAVE_MAP:
                    if env.game == Scenario.GAME_DEEP_RTS:
                        dire = os.path.expanduser(
                            '~') + os.path.sep + 'urnai_maps' + os.path.sep + 'drts' + os.path.sep \
                               + str(step)
                        try:
                            os.makedirs(dire)
                        ...
                        dire = os.path.expanduser(
                            '~') + os.path.sep + 'urnai_maps' + os.path.sep + 'sc2' + os.path.sep \
                               + str(step)
                        try:
                            os.makedirs(dire)
                        except FileExistsError:
